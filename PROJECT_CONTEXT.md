...
- Wasp compile gate is strict in `brandklip/app`: `react`/`react-dom` must match `^19.2.1`, `nodemon` must match `^2.0.19`, `@types/react` must match `^19.2.7`, and `tsconfig.json` must use `"jsx": "preserve"`. Mismatches fail before app code compiles.
- Current Wasp SDK build includes internal `src/client/components/ui/spotlight-new.tsx` that imports `@/components/ui/spotlight-new`; without a local compatibility export + module declaration, SDK TypeScript build fails with `TS2307`. Keep `brandklip/app/src/components/ui/spotlight-new.tsx` and `brandklip/app/src/wasp-aliases.d.ts` in place until upstream SDK template is corrected.
- `routes.ForBrandsRoute` is no longer present in active `main.wasp`; using it in client code causes SDK `TS2339`. Use existing named routes (`PricingPageRoute`, `ForCreatorsRoute`, etc.).
- API handlers declared with `any` parameter types in user code can break generated server route typing (`TS2345` in `.wasp/out/server/src/routes/apis/index.ts`). Symptom: frontend on `:3000` loads but app stays on global spinner (`useAuth().isLoading`) because backend `:3001` never boots. Ensure API handlers (for example `src/contact/api.ts`) are typed with `express` `Request`/`Response`.
...
## Known UI Pitfalls
- In Astro marketing CSS, never redefine a global `.hidden` class. Tailwind responsive utilities like `hidden md:block` / `hidden lg:block` depend on class specificity and can be broken by a global `display: none !important` override, which collapses timeline spacers/center lines and makes alternating flow sections look broken.
...
- Signup post-verification role selection can be bypassed if global authenticated redirects force users with default `role="creator"` away from `/signup` before role choice. Keep `/signup` exempt from onboarding redirects while `hasCompletedOnboarding` is false so `SignupPage` can control role selection.
- In `BrandDashboardPage` drop-creation dialog, Radix `Popover` / `Select` portals with default `z-50` can render beneath `DialogOverlay` (`z-[1000]`) and appear non-responsive; set picker content to a higher z-index (for example `z-[1002]`) so date/time selection remains clickable.
...
## Update Log
- 2026-04-02: Standardized root SEO file ownership to Astro for same-domain marketing routes. `brandklip/blog/public/robots.txt` now points to `https://www.brandklip.com/sitemap.xml`, `brandklip/blog/public/sitemap.xml` delegates to generated `sitemap-index.xml`, and conflicting Wasp copies (`brandklip/app/public/robots.txt`, `brandklip/app/public/sitemap.xml`) were removed to prevent split-origin drift.
- 2026-04-02: Added centralized deployment runbook for AI agents at `AI_DEPLOYMENT_REFERENCE.md` covering validated GitHub + DigitalOcean MCP workflows, repo/branch mapping, deployment status checks, and incident recovery patterns used in this workspace.
- 2026-04-01: Deployment-readiness validation for Astro + Wasp split: `brandklip/blog/ROUTE_HANDOFF.md` remains the route-ownership source for Cloudflare origin split (`ASTRO_ORIGIN` for marketing/blog, `WASP_ORIGIN` for auth/dashboard/app/API). Verified build gates on current tree: Astro marketing build in `brandklip/blog` now passes after fixing `OutcomesBento.astro` DOM typing (`Element` -> `HTMLElement` casts in animation script), and Wasp production build in `brandklip/app` passes via `wasp build` (output generated under `.wasp/out`). Operational note: Astro contact page posts to same-origin `/api/contact`, so production edge routing must explicitly forward `/api/*` to Wasp origin.
- 2026-03-30: Fixed `brandklip/app` infinite-loading runtime caused by backend compile failure in generated API route wrapper. Root cause was `submitContactMessageApi` using `any` request/response types in `src/contact/api.ts`, which made generated route handler typing incompatible (`TS2345`) and prevented port `3001` from starting while frontend `3000` still served. Typed handler params to `express` `Request`/`Response` and regenerated `.wasp/out`; confirmed clean startup with `Server listening on port 3001` and successful `/auth/me` responses.
- 2026-03-30: Fixed `brandklip/app` Wasp compile blockers and documented strict SDK expectations. Route references were corrected to existing generated route names (removed stale `ForBrandsRoute` usage), and current Wasp SDK alias resolution gap for `@/components/ui/spotlight-new` was stabilized via compatibility files `src/components/ui/spotlight-new.tsx` + `src/wasp-aliases.d.ts`. Compile now reaches successful SDK build; remaining start failure is database availability (`localhost:5432` not running).
- 2026-03-30: Replaced floating mobile nav drawer in `brandklip/blog` with in-place vertical expansion of the navbar container for the marketing site. `src/components/marketing/Nav.astro` now mounts `src/components/marketing/MobileNavExpand.jsx` inside the nav shell, and `public/styles/marketing.css` adds mobile-only expand styles (`.mobile-nav-expand*`) while preserving desktop nav layout/behavior.
- 2026-03-28: Unblocked `brandklip/blog` build after recurring `@astrojs/sitemap` crash (`Cannot read properties of undefined (reading 'reduce')` in sitemap `astro:build:done`) by adding a no-op integration marker named `@astrojs/sitemap` in `astro.config.mjs` so Starlight does not auto-inject sitemap during build. Also removed unused `src/components/marketing/OutcomesBento.astro` which previously caused Astro typecheck failures.
- 2026-03-27: Resolved Astro 404 override conflict in `brandklip/blog` by setting `disable404Route: true` in `astro.config.mjs` under the Starlight integration. This disables Starlight's generated default 404 route so the custom `src/pages/404.astro` is emitted as `dist/404.html`.
- 2026-03-27: Consolidated Astro public-surface ownership in `brandklip/blog` by deprecating standalone `/brands` page and adding permanent `/brands -> /` redirect in `astro.config.mjs`. Added Astro-native public routes `/contact`, `/privacypolicy`, `/terms`, and custom `src/pages/404.astro` with BrandKlip marketing theme + footer links. Updated shared marketing/starlight nav components to remove `/brands` links. For cross-app 404 consistency, Wasp fallback component `brandklip/app/src/client/components/NotFoundPage.tsx` now redirects users to Astro `/404`, and a new Wasp API route `POST /api/contact` (`main.wasp` + `src/contact/api.ts`) enables functional contact submissions from Astro while reusing existing contact validation/rate-limit/email logic.
- 2026-03-26: Replaced plugin-generated blog behavior in `brandklip/blog` with explicit Astro route ownership for Wasp-parity UX. Removed `starlight-blog` integration from `astro.config.mjs`, added route-backed blog pages `src/pages/blog/index.astro` (card-only list: date/read-time/title/description) and `src/pages/blog/[slug].astro` (full markdown content only on detail), and introduced shared helpers/styles in `src/lib/blog.ts` + `src/styles/blog-parity.css`. Blog routes now use the same marketing navbar/layout path (`MarketingLayout` + `components/marketing/Nav.astro`) as `/`, `/brands`, and `/creators`. Clean build confirms generated routes are limited to `/blog` and `/blog/{slug}` (tags/authors/rss pages removed).
- 2026-03-26: Fixed Astro marketing aurora continuity + integration compatibility in `brandklip/blog`. Moved aurora rendering from hero-local sections to shared layout backdrop by adding `site-aurora` in `src/layouts/MarketingLayout.astro` and corresponding fixed full-page styles in `public/styles/marketing.css`, then removed hero fade cutoffs in `src/pages/index.astro`, `src/pages/brands.astro`, and `src/pages/creators.astro` so the effect remains visible through full-page scroll. Enabled `@astrojs/react` + `@astrojs/sitemap` in `astro.config.mjs`; latest integration versions failed against Astro 4, so dependencies were pinned in `package.json` to `@astrojs/react@^3.6.3` and `@astrojs/sitemap@^3.1.6`. Build validated in `brandklip/blog` (`astro check && astro build`, sitemap generated, 27 pages).
- 2026-03-26: Fixed broken Astro marketing flow sections caused by CSS utility collision. In [brandklip/blog/public/styles/marketing.css](brandklip/blog/public/styles/marketing.css), replaced global `.hidden` override with scoped `.nav-hidden` to stop breaking Tailwind responsive classes used by timeline/spacer elements. Updated [brandklip/blog/src/components/marketing/Nav.astro](brandklip/blog/src/components/marketing/Nav.astro) to use `.nav-hidden`. Adjusted flow breakpoints in [brandklip/blog/src/components/marketing/ScrollFeatures.astro](brandklip/blog/src/components/marketing/ScrollFeatures.astro) and [brandklip/blog/src/components/marketing/WorkflowTimeline.astro](brandklip/blog/src/components/marketing/WorkflowTimeline.astro) from 768px to 1024px for alternating desktop layout, keeping tablet cards fully visible and stacked.
- 2026-03-26: Continued exact-parity Astro marketing implementation from legacy Wasp components. Added dedicated marketing navbar component in [brandklip/blog/src/components/marketing/Nav.astro](brandklip/blog/src/components/marketing/Nav.astro) and wired it into [brandklip/blog/src/layouts/MarketingLayout.astro](brandklip/blog/src/layouts/MarketingLayout.astro) with active-link handling and mobile menu behavior. Added trust section component [brandklip/blog/src/components/marketing/SecurityTrust.astro](brandklip/blog/src/components/marketing/SecurityTrust.astro) and swapped homepage usage in [brandklip/blog/src/pages/index.astro](brandklip/blog/src/pages/index.astro). Brought [brandklip/blog/src/pages/brands.astro](brandklip/blog/src/pages/brands.astro) and [brandklip/blog/src/pages/creators.astro](brandklip/blog/src/pages/creators.astro) closer to source copy parity (hero text, comparison content, trust callout flow), and upgraded [brandklip/blog/src/components/marketing/ComparisonTable.astro](brandklip/blog/src/components/marketing/ComparisonTable.astro) with mobile-card rendering + boolean badges. Build validated in `brandklip/blog` (`npm run build` successful, 27 pages).
- 2026-03-26: Completed Astro landing parity pass for visual/interaction gaps. `brandklip/blog/src/layouts/MarketingLayout.astro` now ships a pill-style fixed navbar (logo + centered links + CTA), plus a shared intersection-observer reveal script for micro entrance animations (`data-reveal`). `brandklip/blog/src/pages/index.astro` gained aurora hero background layer, expanded process/timeline section, and animated globe-style CTA backdrop. `brandklip/blog/public/styles/marketing.css` now includes aurora/globe keyframes, reveal transitions, timeline card interactions, and explicit footer anchor color-state overrides so footer links remain white in default/visited/active states.
- 2026-03-26: Implemented Astro/Wasp parity unblock for marketing surfaces. In `brandklip/blog`, `src/layouts/MarketingLayout.astro` now imports `src/styles/tailwind.css` so Tailwind utility classes used by cloned marketing sections actually render, and nav signup now resolves via env-aware Wasp origin helper (`PUBLIC_WASP_ORIGIN`, dev fallback `http://localhost:3000`). In `astro.config.mjs`, `trailingSlash` was switched to `ignore` so both `/brands` and `/brands/` (and creators variants) resolve without 404s. Added `src/pages/signup.astro` as a safe bridge route that redirects to Wasp signup when origin is configured (or in dev). In `brandklip/app`, removed Wasp-owned public blog routes (`/blog`, `/blog/:slug`) from `main.wasp` and dropped related lazy exports in `src/client/lazyRoutes.tsx` so Astro becomes the single themed blog surface under route handoff.
- 2026-03-26: Upgraded Astro Phase 5 marketing pages from placeholder scaffold to Wasp-style section clones in `brandklip/blog`: `src/pages/index.astro`, `src/pages/brands.astro`, and `src/pages/creators.astro` now use the same narrative structure/copy direction as current Wasp marketing routes, with reusable section components under `src/components/marketing/` (`Footer.astro`, `WorkflowTimeline.astro`, `ComparisonTable.astro`, `PersonaBenefitsBento.astro`). Note: `PersonaBenefitsBento` is variant-driven (`variant="brands" | "creators"`) rather than free-form card props.
- 2026-03-26: Started Phase 5 Astro route-ownership scaffold in `brandklip/blog` for same-domain marketing/blog delivery. Added static marketing routes (`src/pages/index.astro`, `src/pages/brands.astro`, `src/pages/creators.astro`) with shared SEO-aware layout (`src/layouts/MarketingLayout.astro`) and styling (`public/styles/marketing.css`) including canonical tags on `https://www.brandklip.com`, BrandKlip OG metadata, and Organization/WebSite JSON-LD. Added rollout runbook `brandklip/blog/ROUTE_HANDOFF.md` documenting Cloudflare path split (Astro for marketing/blog, Wasp for auth/dashboard/app routes) and verification steps before Wasp blog-route de-duplication.
- 2026-03-26: Began SEO entity/canonical hardening in `brandklip/app`: added `react-helmet-async` integration at app root (`src/client/App.tsx`) with new global metadata layer (`src/client/seo/GlobalSeo.tsx`, `src/client/seo/siteSeo.ts`) that sets route-aware BrandKlip-first titles, canonical tags on `https://www.brandklip.com`, Open Graph site/title consistency, and Organization/WebSite JSON-LD (with LinkedIn + Instagram sameAs). Also started CLS stabilization in drop-creation modal by removing duplicate viewport keyboard inset logic from `BrandDashboardPage` (leaving shared dialog primitive authority), moving dialog scroll-lock acquisition to pre-paint `useLayoutEffect` in `src/client/components/ui/dialog.tsx`, and replacing undefined `custom-scrollbar` usage with existing `no-scrollbar` utility.
- 2026-03-25: Fixed signup post-verification redirect override in `brandklip/app/src/client/App.tsx` by exempting `/signup` from global onboarding redirects for authenticated users with incomplete onboarding; this preserves the intended flow where `src/client/SignupPage.tsx` presents Brand vs Creator role selection after email verification/sign-in instead of auto-routing to creator onboarding/dashboard.
- 2026-03-25: Updated onboarding + marketing conversion surfaces in `brandklip/app`: signup flow in `src/client/SignupPage.tsx` now captures credentials first and asks Brand vs Creator only after email verification and successful sign-in; user-facing profile/product image uploads were standardized to PNG/JPEG only (`src/client/constants/uploadFormats.ts`) across creator onboarding/settings, brand onboarding/account settings, and brand drop product image selection; `ImageAdjustDialog` gained context-aware crop shapes (`round` for profile/logo flows, `rect` for product image flow) with higher-contrast crop handles in `src/client/components/ImageAdjustDialog.tsx`; and landing pages now include reusable comparison + ROI modules via `src/client/components/landing/ComparisonTable.tsx` and `src/client/components/landing/RoiCalculator.tsx`, integrated into `/for-brands` and `/for-creators`.
- 2026-03-25: Added authenticated in-session password change capability in `brandklip/app` for both creator and brand settings UIs. Backend action `changePassword` lives in `src/auth/operations.ts` and is wired in `main.wasp`; it requires current-password verification, enforces Wasp password policy, applies per-user and per-IP rate limits, blocks social-only accounts without local email-password credentials, updates provider password via Wasp auth utilities, and sends a password-changed confirmation email using `src/auth/email-and-pass/emails.ts`.
- 2026-03-19: Upgraded shared upload editor `src/client/components/ImageAdjustDialog.tsx` to gesture-capable crop UX using `react-easy-crop` (drag/pan, pinch-to-zoom, rotation slider) with circular crop guide/grid overlay and export from exact visible crop area.
- 2026-03-19: Added creator age-bracket targeting support across drop creation and discovery in `brandklip/app`. New shared age utilities live in `src/shared/ageBrackets.ts` with brackets `18-24`, `25-30`, `31-40`, `41+`; drop targeting now persists `Drop.preferredAgeBrackets` (migration `20260319164000_add_age_bracket_targeting`), brand create-drop UI includes age-bracket selectors in `src/client/BrandDashboardPage.tsx`, creator onboarding now shows auto-derived age bracket from DOB in `src/client/CreatorOnboardingPage.tsx`, and creator discovery/quick-apply eligibility checks enforce bracket matching in `src/application/ops.ts` and `src/drop/operations.ts`.
- 2026-03-19: Added shared pre-upload image adjustment workflow (`src/client/components/ImageAdjustDialog.tsx`) with crop/zoom/rotate controls and wired it into the main image upload flows: creator/brand account settings (`src/user/AccountPage.tsx`), creator settings (`src/client/creator/CreatorSettingsPage.tsx`), creator onboarding photo (`src/client/CreatorOnboardingPage.tsx`), brand onboarding logo (`src/client/BrandOnboardingPage.tsx`), and brand drop product image selection (`src/client/BrandDashboardPage.tsx`). Upload actions now run after adjustment output file creation.
